...
```{r,include=FALSE, warning = FALSE, message = FALSE}
# setwd("~/Spatial_Analysis_R/naturewave/ExtData/")


# 
# setwd("~/Spatial_Analysis_R/naturewave/extdata/")
# 
# read_csv(system.file("naturewave/extdata/DF_Fulltable_CBIND.csv", 
#                                 package = "naturewave"))


setwd(dir = "../naturewave/extdata/")
DF <- read.csv("../extdata/DF_Fulltable_CBIND.csv")
...
DF_NDSI_ <- DF %>% dplyr::select(SITE, NDSI_select, Buffer)


DF_Transposed_NDSI <- pivot_wider(DF_transposed, names_from = Buffer, values_from = NDSI_select)

# pivot_longer(-religion, names_to = "Buffer", values_to = "Imperv")


# write.csv(DF_Transposed_NDSI, file = "DF_Transposed_NDSI.csv")
# my_metric3
# 
```
...
setwd("../naturewave/extdata/sample_sites/")
sample_points_f <- st_read("sample_sites_projected.shp")
...
imperv <- raster("../naturewave/extdata/Amherst_CAPS2011/imperv/imperv.tif", crs = crs(sample_points_f))
plot(imperv)
...
connect <- raster("../naturewave/extdata/Amherst_CAPS2011/connect/connect.tif", crs = crs(sample_points_f))
plot(connect)
plot(sample_points_f, col = "red", add = TRUE)
plot(AOI, add = TRUE)
st_crs(connect)
...
structure <- raster("../naturewave/extdata/Amherst_CAPS2011/structure/structure.tif", crs = crs(sample_points_f))
plot(structure)
plot(sample_points_f, col = "red", add = TRUE)
plot(AOI, add = TRUE)
st_crs(structure)
...
#import connectedness raster file
traffic <- raster("../naturewave/extdata/Amherst_CAPS2011/traffic/traffic.tif", crs = crs(sample_points_f))
plot(traffic)
...
Calculate area mean of landscape core area by buffer using lsm_l_area_mn
...
plot(imperv_cropped__focal, main ="Circular MW Mean") 
...
imperv_cropped__focal_mean <-  focal(imperv_cropped, w = fw_fixed, na.rm = TRUE, fun = mean)
...
plot(imperv_cropped__focal_mean, main ="Circular MW Mean with `fun= mean` specified")
...
plot(imperv_cropped__focal_sum, main ="Circular MW correct without ")
...
Imperv_connect_Struct_NDSI <- buffs_extract_zonal_Impervious_pivotted %>% mutate(Connectdness = buffs_extract_zonal_connect_pivoted$Connectedness, Structure  = buffs_extract_zonal_structure_pivoted$Structure, NDSI = DF_transposed$NDSI_select, Biophony = DF$BIO_select, Anthrophony = DF$ANT_select) %>% arrange(SiteName)


write.csv(Imperv_connect_Struct_NDSI, file = "Imperv_connect_Struct_NDSI.csv")
...
Imperv_connect_Struct_Traffic_NDSI <- buffs_extract_zonal_Impervious_pivotted %>% mutate(Connectdness = buffs_extract_zonal_connect_pivoted$Connectedness, Structure  = buffs_extract_zonal_structure_pivoted$Structure, Traffic = buffs_extract_zonal_traffic_pivoted$Traffic, NDSI = DF_transposed$NDSI_select, Biophony = DF$BIO_select, Anthrophony = DF$ANT_select) %>% arrange(SiteName)



write.csv(Imperv_connect_Struct_Traffic_NDSI, file = "Imperv_connect_Struct_Traffic_NDSI.csv")
...
plot(imperv_cropped__focal_mean, main ="Circular MW Mean with function speficied")
plot(imperv_cropped__focal, main ="Circular MW correct without ")
...
plot(imperv_cropped__focal_mean, main ="Circular MW Mean with function speficied", ext = extent(buff500_firstsite))
buffs_extract[[1]]$sites %>% plot(col = NA, add=TRUE, border = "red")
plot(imperv_cropped__focal, main ="Circular MW correct without", ext = extent(buff500_firstsite))
buffs_extract[[1]]$sites %>% plot(col = NA, add=TRUE, border = "red")
...
chirps1_focmu5 <- focal(x = imperv_cropped, w = wmat, na.rm = TRUE) 
