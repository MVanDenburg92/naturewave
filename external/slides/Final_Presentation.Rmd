...
linear_rsquared_new <- system.file("extdata/linear_rsquared_traffic_included.csv", package = "naturewave")

linear_rsquared_new <- read.csv(linear_rsquared_new)
...
r_path <- dir("./data/Amherst_CAPS2011", full.names = TRUE, pattern = ".tif")
# landcover <- lapply(unname(r_path), function(x) {
#   r <- raster(x)
#   crs(r) <- crs(pts)
#   r
# })
