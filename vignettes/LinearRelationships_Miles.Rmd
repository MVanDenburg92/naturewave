...
lc1500 <- lapply(1:4, function(x) {  # x <- 1
  f <- paste0("../external/data/Amherst_CAPS2011", names(lc_stack)[x], "_", radius, ".tif")
...
tit1 <- "Map of 1500 (m) Buffer "

lapply(1:4, function(x){
  plot(lc1500_stacked[[x]], axes = FALSE, box = FALSE, main =  oce::titleCase(c(tit,gsub(".tif", "", basename(f[x])))))
  plot(st_geometry(sample_points_f), add = TRUE)
})
...
tit2 <- "Map of 2000 (m) Buffer "

# Plotting out the results
lapply(1:4, function(x){
  plot(lc2000_stacked[[x]], axes = FALSE, box = FALSE, main =  oce::titleCase(c(tit2,gsub(".tif", "", basename(f[x])))))
  plot(st_geometry(sample_points_f), add = TRUE)
})
...
tit3 <- "Map of 2500 (m) Buffer"

# Plotting out the results
lapply(1:4, function(x){
  plot(lc2500_stacked[[x]], axes = FALSE, box = FALSE, main =  oce::titleCase(c(tit3, gsub(".tif", "", basename(f[x])))))
  plot(st_geometry(sample_points_f), add = TRUE)
})
...
geospaar::plot_noaxes(imperv_cropped__focal_mean_fixed, main ="Circular MW Mean with `fun= mean` specified")#plot single mw mean 
...
pts_2000 <- cbind(pts1, raster::extract(lc2000_stacked, pts1))  # buffer points 1500
