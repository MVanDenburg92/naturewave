...
path(./external/data, ext = "csv")


setwd(dir = "/external/data/")
DF <- read.csv("external/data/DF_Fulltable_CBIND.csv")

dem <- getData(name = "DF_Fulltable_CBIND", path = "./external/data")

# DF <- read.csv("./external/DF_Fulltable_CBIND.csv")
...
setwd("../naturewave/external/data/sample_sites/")
sample_points_f <- st_read("sample_sites_projected.shp")
...
imperv <- raster("../naturewave/external/data/Amherst_CAPS2011/imperv/imperv.tif", crs = crs(sample_points_f))
plot(imperv)
...
connect <- raster("../naturewave/external/data/Amherst_CAPS2011/connect/connect.tif", crs = crs(sample_points_f))
plot(connect)
plot(sample_points_f, col = "red", add = TRUE)
plot(AOI, add = TRUE)
st_crs(connect)
...
structure <- raster("../naturewave/external/data/Amherst_CAPS2011/structure/structure.tif", crs = crs(sample_points_f))
plot(structure)
plot(sample_points_f, col = "red", add = TRUE)
plot(AOI, add = TRUE)
st_crs(structure)
...
#import connectedness raster file
traffic <- raster("../naturewave/external/data/Amherst_CAPS2011/traffic/traffic.tif", crs = crs(sample_points_f))
plot(traffic)
...
plot(imperv_cropped__focal_mean_fixed, main ="Circular MW Mean with function speficied")
, ext = extent(buff3000_secondsite))
